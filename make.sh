...
case "$(uname -s)" in
	...
	Darwin)
		...
		DOCKER_USER=exporter
		DOCKER_GROUP=exporter
...
mkdir -p "${XDG_CACHE_HOME}/exporter"
...
docker run \
	...
	${TTY} \
	--interactive \
	--volume "${GIT_ROOT}:${DOCKER_HOME}/exporter" \
	--volume "${XDG_CACHE_HOME}/exporter:${DOCKER_XDG_CACHE_HOME}/exporter" \
	--env "XDG_CACHE_HOME=${DOCKER_XDG_CACHE_HOME}" \
	${GO_ENV_ARGS} \
	--workdir ${DOCKER_HOME}/exporter \
	${DOCKER_IMAGE} \
